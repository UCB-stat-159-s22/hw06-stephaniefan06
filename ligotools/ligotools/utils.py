...
# This function "whitens" the noises that have high and low frequencies
# by dividing it by the noise amplitude spectrum, in the fourier domain, 
# suppressing the extra noise at low frequencies and at the spectral lines, 
# to better see the weak signals in the most sensitive band.

def whiten(strain, interp_psd, dt):
	
	# This function takes in the strain, power spectral densities(PSDs),
	# and the time sample interval(dt).
	# It returns the whitened sound data.
...
# function to keep the data within integer limits, 
# and write to wavfile in the audio directory

def write_wavfile(filename,fs,data):
	
	# This function takes in the filename selected for to-save data,
	# sampling rate, and the data
	# It saves the wav file into the audo directory.
...
# The resulting sound file will be noticibly more high-pitched, 
# and the signal will be easier to hear.
def reqshift(data,fshift=100,sample_rate=4096):
	
    # This function takes in the signal data,
    # shifts it by frequency=100 and sample_rate=4096.
    # It returns the shifted signal data.
...
# Plot the results
def plot_detectors(det, time, timemax, SNR, pcolor, tevent, data, strain_whitenbp, template_match, template_fft, datafreq, d_eff, freqs, data_psd):
